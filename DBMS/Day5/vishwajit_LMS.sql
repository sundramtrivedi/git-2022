...
-- Count of suppliers against their respective category of their email amounts
-- eg gmail/yahoo

SELECT SUBSTR(EMAIL, INSTR(EMAIL, '@') + 1) Company, COUNT(*) FROM LMS_SUPPLIERS_DETAILS
WHERE EMAIL LIKE '%@gmail.com' OR EMAIL LIKE '%@yahoo.com'
GROUP BY SUBSTR(EMAIL, INSTR(EMAIL, '@') + 1);


-- Count of suppliers against their respective category of their email amounts
-- eg gmail/yahoo and others

SELECT DECODE(SUBSTR(EMAIL, INSTR(EMAIL, '@') + 1), 'gmail.com', 'Gmail', 'yahoo.com', 'Yahoo', 'Others') Company, COUNT(*)
...
SELECT CATEGORY, COUNT(*) Total_Books
...
-- Name of the supplier along with his contact number with a Flag 'Y', 'N' OR 'NA'
--      Y= HE HAD SUPPLIED A BOOK WHICH HAS BEEN ISSUED TO TWO OR MORE MEMBERS
--      N= HE AHD SUPPLIED A BOOK WHICH HAS BEEN ISSUED TO LES THAN TWO MEMBERS
--      NA=IF HE  HASNOT SUPPLIED A BOOK
...
-- List the book name and issuance date, date of expected return such that the books that are to be returned at the earlist are seen at the top

SELECT book_title, date_issue, date_return
FROM LMS_BOOK_DETAILS
LEFT JOIN LMS_BOOK_ISSUE
...
-- List all the members such that the longest duration of membership member appears at the top
...
-- Provide the supplier name, their city, no_of_books supplied based on the person who has supplied most books comes at the top

SELECT SUPPLIER_NAME, ADDRESS, (LMS_BOOK_DETAILS.SUPPLIER_ID) Counting, COUNT(*)
FROM LMS_SUPPLIERS_DETAILS
INNER JOIN LMS_BOOK_DETAILS
ON (LMS_SUPPLIERS_DETAILS.SUPPLIER_ID = LMS_BOOK_DETAILS.SUPPLIER_ID)
GROUP BY (LMS_BOOK_DETAILS.SUPPLIER_ID), SUPPLIER_NAME, ADDRESS
ORDER BY COUNT(*) DESC;


SELECT SUPPLIER_NAME, ADDRESS, (LMS_BOOK_DETAILS.SUPPLIER_ID) Counting, COUNT(LMS_BOOK_DETAILS.SUPPLIER_ID)
FROM LMS_SUPPLIERS_DETAILS
LEFT JOIN LMS_BOOK_DETAILS
ON (LMS_SUPPLIERS_DETAILS.SUPPLIER_ID = LMS_BOOK_DETAILS.SUPPLIER_ID)
GROUP BY (LMS_BOOK_DETAILS.SUPPLIER_ID), SUPPLIER_NAME, ADDRESS
...
-- Provide the books codes and book names based on the least issued book comes at the top
-- Note: we would have books who have not been issued at the bottom of the output

SELECT LMS_BOOK_DETAILS.BOOK_CODE, LMS_BOOK_DETAILS.BOOK_TITLE, COUNT(LMS_BOOK_ISSUE.BOOK_CODE)
FROM LMS_BOOK_DETAILS
LEFT JOIN LMS_BOOK_ISSUE
ON (LMS_BOOK_DETAILS.BOOK_CODE = LMS_BOOK_ISSUE.BOOK_CODE)
GROUP BY LMS_BOOK_DETAILS.BOOK_CODE, LMS_BOOK_DETAILS.BOOK_TITLE, LMS_BOOK_ISSUE.BOOK_CODE
ORDER BY COUNT(*) ASC;



-- Provide the name of the member,total_fine based on the most fined member comes at the top note: we would have not been issued books at the bottom of the output

SELECT LMS_MEMBERS.MEMBER_NAME, DECODE(SUM(LMS_FINE_DETAILS.FINE_AMOUNT), null, 0, SUM(LMS_FINE_DETAILS.FINE_AMOUNT)) TOTAL
FROM LMS_MEMBERS
LEFT JOIN LMS_BOOK_ISSUE
ON (LMS_MEMBERS.MEMBER_ID = lms_book_issue.MEMBER_ID)
LEFT JOIN LMS_FINE_DETAILS
ON (LMS_BOOK_ISSUE.FINE_RANGE = LMS_FINE_DETAILS.FINE_RANGE)
GROUP BY LMS_MEMBERS.MEMBER_NAME, LMS_MEMBERS.member_id
ORDER BY TOTAL DESC NULLS LAST, LMS_MEMBERS.MEMBER_NAME
...
SELECT LMS_BOOK_DETAILS.RACK_NUM, LMS_BOOK_DETAILS.BOOK_EDITION, COUNT(LMS_BOOK_DETAILS.RACK_NUM)
FROM LMS_BOOK_DETAILS
...
INNER JOIN LMS_MEMBERS ON (LMS_BOOK_ISSUE.MEMBER_ID = LMS_MEMBERS.MEMBER_ID)
WHERE LMS_BOOK_DETAILS.BOOK_EDITION >= 3 AND ((LMS_SUPPLIERS_DETAILS.EMAIL) LIKE '%gmail.com' OR (LMS_SUPPLIERS_DETAILS.EMAIL) LIKE '%redif%') AND (LMS_MEMBERS.MEMBERSHIP_STATUS) = 'Permanent'
GROUP BY LMS_BOOK_DETAILS.RACK_NUM, LMS_BOOK_DETAILS.BOOK_EDITION
ORDER BY COUNT(LMS_BOOK_DETAILS.RACK_NUM);
...
SELECT LMS_MEMBERS.MEMBER_ID, LMS_MEMBERS.MEMBER_NAME, LMS_MEMBERS.CITY, LMS_SUPPLIERS_DETAILS.ADDRESS SUPPLIERS_ADDRESS, LMS_BOOK_DETAILS.DATE_ARRIVAL, COUNT(DISTINCT LMS_MEMBERS.MEMBER_ID) COUNT
FROM LMS_MEMBERS
...
INNER JOIN LMS_SUPPLIERS_DETAILS ON (LMS_BOOK_DETAILS.SUPPLIER_ID = LMS_SUPPLIERS_DETAILS.SUPPLIER_ID)
WHERE LMS_MEMBERS.CITY IN ('DELHI', 'Delhi', 'MUMBAI', 'Mumbai', 'PUNE', 'Pune') AND LMS_SUPPLIERS_DETAILS.ADDRESS IN ('DELHI', 'Delhi', 'MUMBAI', 'Mumbai', 'PUNE', 'Pune', 'CHENNAI', 'Chennai') 
AND LMS_BOOK_DETAILS.DATE_ARRIVAL > TO_DATE('1-JUL-2000', 'DD-MON-YYYY')
GROUP BY LMS_MEMBERS.MEMBER_ID, LMS_MEMBERS.MEMBER_NAME, LMS_MEMBERS.CITY,  LMS_SUPPLIERS_DETAILS.ADDRESS, LMS_BOOK_DETAILS.DATE_ARRIVAL
ORDER BY COUNT(LMS_MEMBERS.MEMBER_ID);
...
Insert into LMS_MEMBERS (MEMBER_ID,MEMBER_NAMELMS_BOOK_DETAILS.SUPPLIER_ID = LMS_SUPPLIERS_DETAILS.SUPPLIER_ID),CITY,DATE_REGISTER,DATE_EXPIRE,MEMBERSHIP_STATUS) values ('LM023','Ritesh','Bangalore',to_date('12-06-18','DD-MM-RR'),to_date('17-08-20','DD-MM-RR'),'Permanent');
