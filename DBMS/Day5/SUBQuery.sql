...
WHERE LMS_BOOK_DETAILS.SUPPLIER_ID IN (SELECT SUPPLIER_ID FROM LMS_SUPPLIERS_DETAILS WHERE ADDRESS IN ('DELHI', 'Delhi', 'MUMBAI', 'Mumbai', 'CHENNAI', 'Chennai') AND LMS_BOOK_DETAILS.SUPPLIER_ID = LMS_SUPPLIERS_DETAILS.SUPPLIER_ID);

-- WITH EXISTS --

SELECT *
FROM LMS_BOOK_DETAILS
WHERE EXISTS (SELECT SUPPLIER_ID FROM LMS_SUPPLIERS_DETAILS WHERE ADDRESS IN ('DELHI', 'Delhi', 'MUMBAI', 'Mumbai', 'CHENNAI', 'Chennai') AND LMS_BOOK_DETAILS.SUPPLIER_ID = LMS_SUPPLIERS_DETAILS.SUPPLIER_ID);

-- Name of the member whose has issued a book, that same book supplier belong to the same city as that of the member

SELECT MEMBER_ID, MEMBER_NAME
FROM LMS_MEMBERS
WHERE LMS_MEMBERS.MEMBER_ID IN  (SELECT MEMBER_ID FROM LMS_BOOK_ISSUE WHERE BOOK_CODE IN 
                                (SELECT BOOK_CODE FROM LMS_BOOK_DETAILS WHERE SUPPLIER_ID IN 
                                (SELECT SUPPLIER_ID FROM LMS_SUPPLIERS_DETAILS WHERE LMS_SUPPLIERS_DETAILS.ADDRESS = LMS_MEMBERS.CITY)));

-- WITH EXISTS --

SELECT MEMBER_ID, MEMBER_NAME
FROM LMS_MEMBERS
WHERE EXISTS    (SELECT MEMBER_ID FROM LMS_BOOK_ISSUE WHERE BOOK_CODE IN 
                (SELECT BOOK_CODE FROM LMS_BOOK_DETAILS WHERE SUPPLIER_ID IN 
                (SELECT SUPPLIER_ID FROM LMS_SUPPLIERS_DETAILS WHERE LMS_SUPPLIERS_DETAILS.ADDRESS = LMS_MEMBERS.CITY)) AND LMS_BOOK_ISSUE.MEMBER_ID = LMS_MEMBERS.MEMBER_ID);

SELECT MEMBER_ID, MEMBER_NAME, CITY
FROM LMS_MEMBERS
WHERE EXISTS    (SELECT * FROM LMS_BOOK_ISSUE, LMS_BOOK_DETAILS, LMS_SUPPLIERS_DETAILS
                WHERE (LMS_MEMBERS.MEMBER_ID = LMS_BOOK_ISSUE.MEMBER_ID)
                AND (LMS_BOOK_ISSUE.BOOK_CODE = LMS_BOOK_DETAILS.BOOK_CODE)
                AND (LMS_BOOK_DETAILS.SUPPLIER_ID = LMS_SUPPLIERS_DETAILS.SUPPLIER_ID)
                AND (LMS_MEMBERS.CITY = LMS_SUPPLIERS_DETAILS.ADDRESS));

-- Book code and name of the book that have been issued

SELECT BOOK_CODE, BOOK_TITLE
FROM LMS_BOOK_DETAILS
WHERE LMS_BOOK_DETAILS.BOOK_CODE IN (SELECT BOOK_CODE FROM LMS_BOOK_ISSUE WHERE LMS_BOOK_DETAILS.BOOK_CODE = LMS_BOOK_ISSUE.BOOK_CODE);

-- WITH EXISTS --

SELECT BOOK_CODE, BOOK_TITLE
FROM LMS_BOOK_DETAILS
WHERE EXISTS (SELECT BOOK_CODE FROM LMS_BOOK_ISSUE WHERE LMS_BOOK_DETAILS.BOOK_CODE = LMS_BOOK_ISSUE.BOOK_CODE);

-- Book issue details of the member who belongs to Pune / Membai / Chennai

SELECT *
FROM LMS_BOOK_ISSUE
WHERE LMS_BOOK_ISSUE.MEMBER_ID IN  (SELECT MEMBER_ID FROM LMS_MEMBERS 
                                    WHERE LMS_BOOK_ISSUE.MEMBER_ID = LMS_MEMBERS.MEMBER_ID 
                                    AND CITY IN ('PUNE', 'Pune', 'MUMBAI', 'Mumbai', 'CHENNAI', 'Chennai'));

-- WITH EXISTS --

SELECT *
FROM LMS_BOOK_ISSUE
WHERE EXISTS  (SELECT MEMBER_ID FROM LMS_MEMBERS 
                WHERE LMS_BOOK_ISSUE.MEMBER_ID = LMS_MEMBERS.MEMBER_ID 
                AND CITY IN ('PUNE', 'Pune', 'MUMBAI', 'Mumbai', 'CHENNAI', 'Chennai'));
                                    
                                    
-- Memeber who have never issued a book

SELECT MEMBER_ID, MEMBER_NAME
FROM LMS_MEMBERS
WHERE LMS_MEMBERS.MEMBER_ID NOT IN (SELECT MEMBER_ID FROM LMS_BOOK_ISSUE 
                                WHERE LMS_MEMBERS.MEMBER_ID = LMS_BOOK_ISSUE.MEMBER_ID);
...
SELECT *
FROM LMS_MEMBERS
WHERE NOT EXISTS (SELECT MEMBER_ID FROM LMS_BOOK_ISSUE 
                                WHERE LMS_MEMBERS.MEMBER_ID = LMS_BOOK_ISSUE.MEMBER_ID);
