...
select
MEMBER_NAME "registered number name",
CITY,
DATE_EXPIRE,
sysdate,
...
--- select MEMBER_NAME, CITY, DATE_REGISTER, DATE_EXPIRE, (to_date(sysdate) - to_date(DATE_REGISTER)) DAYS_COMPLETED, (to_date(sysdate) - to_date(DATE_EXPIRE)) DAYS_REMAINING from LMS_MEMBERS;
...
select * from LMS_SUPPLIERS_DETAILS where upper(ADDRESS) = upper('CHENNAI') OR upper(ADDRESS) = upper('DELHI') OR upper(ADDRESS) = upper('AHMEDABAD');

select * from LMS_BOOK_DETAILS;

select 
BOOK_TITLE, 
EXTRACT (YEAR from PUBLISH_DATE) PUBLISH_YEAR, 
PUBLICATION,  
CATEGORY,
decode (CATEGORY, 'JAVA', 'OBJECT ORIENTED Language', 'C', 'Procedural Language') TYPE_OF_LANGUAGE
