...
{
	if (argc < 3)
	{	// using command line argument user will enter below 2 arguments
		printf("Usage: %s <serv_ip> <serv_port>\n", argv[0]);
		exit(1);
	}
	//variable initialization 
	int cfd, serv_port;
	//serv_port converted to unsigned long as anything we enter on commad line is considered as string
	...
	*/
	...
	{
		perror("connect");
		close(cfd);//cfd form accept api of server
		exit(3);
	}
