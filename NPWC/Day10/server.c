...
int main(int argc, char *argv[])
{
	...
	int sfd, cfd, port_no;
	port_no = strtoul(argv[1], NULL, 10);//string to integer conversion as cmd has string as input
	//INADDR_ANY = strtoul(argv[2], NULL, 10);	
	...
	*/
	...
	if (listen(sfd, 5) < 0)//5=backlog connections
	{
		perror("listen");
		close(sfd);
		exit(4);
	...
	char buf[20] = {0};
	int rec = 0;
	if ((rec = recv(cfd, buf, sizeof(buf), 0)) < 0)
	{
		perror("recv");
		close(cfd);
		close(sfd);
...
printf("Data received in server \n",buf);
	if (send(cfd, buf, rec, 0) < 0)
	{
		perror("send");
		close(cfd);
		close(sfd);
